...
def log_unmatched_instruction(function:str, compiler:str, arch:str, expected:list, actual:list):
...
def validate(input='all', raise_exception=False, log_file=False, keep_tmp=False, compiler=None, architecture=None, silent=True):
        ...
        for c in compilers:
            for a in architectures:
                if f not in validation_set[c][a].keys():
                    ...
                    log_txt += log_unmatched_instruction(f, c, a, validation_set[c][a][f], functions_assembly[c][a][f])
                    ret = -1
